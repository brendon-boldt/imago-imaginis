...
for op in old_graph.get_operations():
	if re.match(".*_[wb]$", op.name):
		tf.Variable(
			initial_value = tf.zeros(op.outputs[0].shape),
			name = op.name,
			#dtype = op.dtype
		)
	else:
		#tf.Operation(op.node_def, new_graph)
		#if op.op_def:
...
#print(opSet)
	...
	#print('~~~ ' + op.name)
	#if op.op_def:
		#print(op.op_def.name)
	#else:
		#print(op)
#op = dgraph.get_operation_by_name("import/conv2d0_w")
#print(op.outputs[0])
#op.outputs[0] = tf.Variable(tf.zeros(op.outputs[0].shape))
#op.outputs[0] = 0
#op = 0
#3print(op.outputs[0])
