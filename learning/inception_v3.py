...
import tensorflow.contrib.slim.python.slim.nets.inception as inception
...
def main():
	...
	t_input = tf.placeholder(np.float32, [None, 299, 299, 3], name='input')
	imagenet_mean = 117.0
	t_preprocessed = tf.expand_dims(t_input-imagenet_mean, 0)

	#model_fn = inception.inception_v3(t_preprocessed)
	
	with slim.arg_scope(inception.inception_v3_arg_scope()):
		model_fn = inception.inception_v3(t_input,num_classes=1001)
	...
	restorer.restore(sess, "./pretrained/inception_v3.ckpt")
 ...
 ############################################################
	...
	def calc_grad_tiled(img, t_grad, tile_size=299):
		...
		for y in range(0, max(h-sz//2, sz),sz):
			for x in range(0, max(w-sz//2, sz),sz):
				#print("%d:%d %d:%d" % (y, y+sz, x,x+sz))
				#y_adjust = max(0,(y+sz) - h)
				#y_adust = 0 if y_adjust > 0 else y_adjust
				#x_adjust = max(0,(x+sz) - w)
				#x_adust = 0 if x_adjust > 0 else x_adjust
				sub = img_shift[y:y+sz,x:x+sz]
				#sub = np.pad(sub, [[0,y_adjust],[0,x_adjust],[0,0]], 'mean')
				#sub = img_shift[y+y_adjust:y+sz+y_adjust,x:x+sz]
				#print(sub.shape)
				g = sess.run(t_grad, {t_input:[sub]})
				#print(g.shape)
				#print("%d:%d %d:%d" % (y, min(h,y+sz), x,min(w,x+sz)))
				grad[y:y+sz,x:x+sz] = g
				#grad[y:min(h,y+sz),x:min(w,x+sz)] = g
	...
	layer = "InceptionV3/InceptionV3/Mixed_6a/Branch_0/Conv2d_1a_1x1/Relu"
